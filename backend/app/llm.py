...
class LlmClient:
    ...
    def answer(self, question: str, citations: list[dict[str, Any]]) -> str | None:
        if not self.enabled:
            return None
        return self._answer_groq(question, citations)

    def _answer_groq(self, question: str, citations: list[dict[str, Any]]) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Groq LLM request failed: HTTP {error.code}: {detail}") from error

        choices = data.get("choices", [])
        if not choices:
            raise RuntimeError(f"Groq LLM returned no choices: {data}")
